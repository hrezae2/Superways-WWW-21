...
double          s_LinkSpeed               = 40.0;
string          SLinkSpeed                 = "40Gbps";
...
uint32_t        queueBytes                = 240e3;      // Number of bytes per queue port
...
double          longNodeFactor            = 0.10;


uint32_t        FlowSizeShort             = 3.45*1024;
uint32_t        IncastFlowSizeLong        = 64 * 1024;
uint32_t        FlowSizeLong              = 2350 * 1024;

bool 		pktspray 		  = 0;
bool		dctcp			  = 1; 
double 		testruntime		  = 0.012;
...
randomShort (void)
{
  ...
  UniformlyRandomly->SetAttribute ("Max", DoubleValue (100));
  if (UniformlyRandomly->GetValue () < 41){
        FlowSizeShort = 1 * 1024;
                                          }
  else if  (40 < UniformlyRandomly->GetValue () && UniformlyRandomly->GetValue () < 51) {
        FlowSizeShort = 2 * 1024;
                                                        }
 else if  (50 < UniformlyRandomly->GetValue () && UniformlyRandomly->GetValue () < 61) {
        FlowSizeShort = 3 * 1024;
                                                        }
  else if  (60 < UniformlyRandomly->GetValue () && UniformlyRandomly->GetValue () < 71) {
        FlowSizeShort = 4 * 1024;
                                                        }
  else if  (70 < UniformlyRandomly->GetValue () && UniformlyRandomly->GetValue () < 81) {
        FlowSizeShort = 5 * 1024;
                                                        }
  else if  (80 < UniformlyRandomly->GetValue () && UniformlyRandomly->GetValue () < 86) {
        FlowSizeShort = 6 * 1024;
                                                        }
  else if  (85 < UniformlyRandomly->GetValue () && UniformlyRandomly->GetValue () < 91) {
        FlowSizeShort = 7 * 1024;
                                                        }
  else {
        FlowSizeShort = 10 * 1024;
        }
}
...
randomLong (void)
{
  ...
  if (UniformlyRandomly->GetValue () < 51){
        FlowSizeLong = 100 * 1024;
                                          }
  else if  (50 < UniformlyRandomly->GetValue () && UniformlyRandomly->GetValue () < 81) {
        FlowSizeLong = 1000 * 1024;
                                                        }
  else {
        FlowSizeLong = 10000 * 1024;
                                                        }
}
...
void Configure_Simulator(){
  ...
  InterArrival_incast = (NumberofIncastAggregators*FlowSizeShort*8*incastDegree) / (load * incastLoadFactor * d_LinkSpeed * NumberofRacks * NumberofSpineSwitches *1000000000);  
...
void Build_Topology()
{
  ...
  p2p.SetDeviceAttribute	("DataRate",  StringValue (SLinkSpeed));
...
//connect incast aggregators to spine switches too.
  for (uint32_t i=19;i<NumberofEndPoints;i+=20){
      for (uint32_t j=0;j<NumberofSpineSwitches-7;j++){
