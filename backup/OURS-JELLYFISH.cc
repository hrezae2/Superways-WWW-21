...
void Build_Topology()
{
 ...
 for (uint32_t i=0;i<NumberofEndPoints;i++){
        ...
        x->SetAttribute ("Max", DoubleValue (19));
        devices = p2p.Install(nLeaf[x->GetInteger()],nEnd[i]);
...
//connect incast aggregators to spine switches too.
 ...
 for (uint32_t i=0;i<NumberofRacks;i++){
    if (i<NumberofRacks-1){
      devices = e2t.Install(nLeaf[i],nLeaf[i+1]);
      interfaces = ipv4.Assign(devices);
      ipv4.NewNetwork();
    }
  }
 ...
 for (uint32_t i=0;i<NumberofSpineSwitches;i++){
        ...
        y->SetAttribute ("Max", DoubleValue (19));
        for (uint8_t j=0;j<NumberofRacks;j++){
         devices = p2p.Install(nLeaf[y->GetInteger()],nSpine[i]);
         interfaces = ipv4.Assign(devices);
         ipv4.NewNetwork();
   }
  }
